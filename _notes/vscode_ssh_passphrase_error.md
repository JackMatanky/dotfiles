...
If it works in Zsh or Nushell but **fails in VS Code**, the issue is likely with the environment not being inherited by GUI apps.
...
If your key is shown and the socket exists, the agent is working.
...
git pull origin main   # Should work without asking for passphrase
```
...
Your SSH key works in VS Code and any GUI app using the same environment. Passphrase prompts no longer appear unless your agent is reset or rebooted.
...
flowchart TD
    ...
    E1 --> F[git pull works in terminal]
