-- ----------------------------------------------------------------------------
-- Filename: ~/dotfiles/nvim/lua/snippets/markdown/base.lua
...
local function generate_table(rows, cols)
  ...
  -- Header row
  for _ = 1, cols do fmt_str = fmt_str .. "| {} " end
  fmt_str = fmt_str .. "|\n"
  for _ = 1, cols do
    table.insert(insert_nodes, i(cell_count)); cell_count = cell_count + 1
  end
...
return {
  ...
  s({
    ...
    {}
    ```]], {
      c(1, { t("python"), t("sql"), t("javascript"), t("typescript"), t("r"), t("markdown"), t("lua") }),
      i(0)
    })
  ),
  s({
